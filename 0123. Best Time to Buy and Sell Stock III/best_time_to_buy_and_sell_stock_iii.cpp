...
// =>
// global[i][j] = max(local[i][j], global[i - 1][j]) 比較 local在第 i 天與 global 在 i - 1 天之利潤(i - 1是因為如果取 i
// 那一定包含到 local[i])
//
// local[i][j] = max(global[i - 1][j - 1], max(diff,0), local[i - 1][j] + diff)
// global[i - 1][j - 1], max(diff,0) => 第 i 天前一天並少一次交易的 global 利潤加上最後一次(i,j)的正交易
// local[i - 1][j] + diff => local[i - 1][j] 的意思比較難理解，在 i 前一天進行 j 次交易後的最大利潤，加上diff，這個diff
// 就是最後一次的交易，所以才取"[j]"，同時不管diff是正是負都必須加上才符合轉移方程的條件
...
int maxProfit(std::vector<int> &prices) {
  ...
  for (auto &n : prices) {
    ...
    sell_2 = std::max(sell_2,
                      buy_2 + n); // 第二次賣出，手上有 buy_2 的錢，賣出 n 所以最後為 buy_2 + n，為了最大利潤所以取 max
  }
